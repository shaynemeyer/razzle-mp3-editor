...
class Mp3:
    ...
    def __init__(self, id3):
        ...
        if not isinstance(id3.tag, type(None)):
            id3.tag.artist = self.normalize_mp3(id3.tag.artist)
            self.artist = id3.tag.artist
            id3.tag.album = self.normalize_mp3(id3.tag.album)
            self.album = id3.tag.album
            id3.tag.title = self.normalize_mp3(id3.tag.title)
            self.title = id3.tag.title
            if hasattr(id3.tag, "best_release_date"):
                if not isinstance(id3.tag.best_release_date, type(None)):
                    self.year = self.normalize_mp3(id3.tag.best_release_date.year)
                else:
...
class TaggerFrame(wx.Frame):
    ...
    def __init__(self):
        super().__init__(None, title="Razzle - MP3 Editor")
    ...
    def create_tool_bar(self):
        ...
        self.Bind(wx.EVT_MENU, self.on_open_folder, add_folder_tool)
