...
class Mp3TagEditorDialog(wx.Dialog):
    ...
    def create_ui(self):
        ...
        self.track_number = wx.TextCtrl(self, value=track_num, size=size)
        self.create_row("Track Number", self.track_number)

        self.artist = wx.TextCtrl(self, value=self.mp3.id3.tag.artist, size=size)
        self.create_row("Artist", self.artist)

        self.album = wx.TextCtrl(self, value=self.mp3.id3.tag.album, size=size)
        self.create_row("Album", self.album)

        self.title = wx.TextCtrl(self, value=self.mp3.id3.tag.title, size=size)
    ...
    def create_row(self, label, text):
        ...
        widgets = [(row_label, 0, wx.ALL, 5), (text, 0, wx.ALL, 5)]
    ...
    def save(self, event):
        ...
        if current_track_num:
            new_track_num = (int(self.track_number.GetValue()), current_track_num[1])
        else:
